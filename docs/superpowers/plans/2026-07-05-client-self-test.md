# QmtClient.self_test() Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a `self_test()` method to `QmtClient` that runs all read-only query interfaces and returns a structured report, replacing `scripts/test_client.py`.

**Architecture:** A module-level `_SELF_TEST_CASES` list of dicts defines test cases. Each case has a `run(client, symbols)` callable. `self_test()` iterates the list, runs each case in a try/except, prints ✓/✗/○ to stdout in real time, and returns a report dict.

**Tech Stack:** Pure Python — no new dependencies. Uses existing `QmtClient` RPC methods and `client.proxy.DownloadTaskHandle`.

## Global Constraints

- `QmtClient.self_test(test_symbols=None, include_bson_risk=False, timeout=30.0) -> dict`
- Only read-only interfaces — no order placement, cancellation, or mutation
- BSON-risk functions (`get_market_data`, `get_local_data`, `get_market_data_ex`) skipped by default; opt-in via `include_bson_risk=True`
- `download_*` functions: verify `DownloadTaskHandle` return type only, do NOT wait for completion
- Trader query methods: skipped if no account configured or trader not connected
- Per-call exceptions → `"fail"` status, do not abort remaining tests
- Functions not in API surface → `"skip"` status (version differences)
...
|---|---|---|
| `client/client.py` | Modify | Add `_SELF_TEST_CASES` constant + `QmtClient.self_test()` method |
| `scripts/test_client.py` | Delete | Superseded by `self_test()` |

---

### Task 1: Add `_SELF_TEST_CASES` constant and `self_test()` method

**Files:**
- Modify: `client/client.py`

**Interfaces:**
- Produces: `QmtClient.self_test(test_symbols=None, include_bson_risk=False, timeout=30.0) -> dict`
- Produces: module-level `_SELF_TEST_CASES: list[dict]`

#### Step 1: Add `import os` to client/client.py

The `self_test()` method reads `QMT_ACCOUNT_ID` from the environment for trader tests.

- [ ] Open `client/client.py`, find the imports block (lines 1–6), add `import os`:
...
`time` is already imported. `os` is new.

#### Step 2: Add `_run_option_detail_batch` helper + `_SELF_TEST_CASES` constant

Insert after `logger = logging.getLogger(__name__)` (line 10), before `class _EventPoller:` (line 13).

- [ ] Insert the helper function and test case registry:
...
# Each entry defines one self-test case.
#   category, name: display grouping and label
#   run(client, symbols) -> result: executed test (may raise)
#   check(result) -> (ok: bool, detail: str): validates result
#   skip_if(client, symbols, opts) -> (should_skip: bool, reason: str) | None
...
_SELF_TEST_CASES = [
    ...
    },
    {
        ...
        "check": lambda r: (all(isinstance(v, int) for v in r),
                            "STOCK_BUY=%d STOCK_SELL=%d MARKET_SH=%d MARKET_SZ=%d" % r),
    },
    # ── instrument ─────────────────────────────────────────────────────
    {
        "category": "instrument",
...
#### Step 3: Add `self_test()` method to `QmtClient`

Insert inside the `QmtClient` class, after `close()` (line 180), before the end of the class.
    ...
    def self_test(self, test_symbols=None, include_bson_risk=False,
        ...
        Args:
            test_symbols: Optional dict overriding default test symbols.
                Keys: sh_stock (default "600000.SH"),
                      sz_stock (default "000001.SZ"),
                      etf     (default "510050.SH"),
                      sector  (default "沪深300"),
                      market  (default "SSE"),
                      account_id (default from QMT_ACCOUNT_ID env var).
            include_bson_risk: If True, also test get_market_data,
                get_local_data, get_market_data_ex (known BSON crash).
            timeout: Not used per-call (rpyc timeout is set at
                connect time). Reserved for future use.
        ...
        for case in _SELF_TEST_CASES:
            ...
            if skip_if:
                should_skip, reason = skip_if(self, symbols, opts)
                if should_skip:
            ...
            try:
                ...
                if category == "download":
                    ...
                    detail = ("task_id=%s" % getattr(result, "task_id", "?")
                ...
                ok = False
                status = "fail"
                # If the function doesn't exist in the xtdata API surface,
                # treat as skip (version difference), not fail
                if isinstance(e, AttributeError) and (
                    ...
                    # Distinguish trader "not connected" from real errors
                    msg = str(e)
            ...
            extra = " (%dms)" % dt_ms if status != "skip" else ""
            line = "  %s %-42s%s" % (symbol, name, extra)
            print(line)
            if detail and status in ("fail", "skip") and "—" not in line:
                # detail already shown inline for skips
                if status == "fail":
                    print("    → %s" % detail)
...
```bash
python -c "import py_compile; py_compile.compile('client/client.py', doraise=True); print('OK')"
```
...
git commit -m "feat: add QmtClient.self_test() method with full query coverage"
```

---

### Task 2: Delete `scripts/test_client.py`

**Files:**
- Delete: `scripts/test_client.py`

- [ ] Delete the file:

```bash
git rm scripts/test_client.py
```

- [ ] Commit:

```bash
git commit -m "chore: remove scripts/test_client.py (superseded by QmtClient.self_test())"
```
...
- [ ] Run syntax check on the modified client.py:

```bash
python -c "import py_compile; py_compile.compile('client/client.py', doraise=True); print('OK')"
```

- [ ] Run the new method's test collection to verify import works:

```bash
python -m pytest tests/test_live_integration.py --collect-only -q
```
Expected: 51 tests collected (no import errors from client module).

- [ ] Confirm `scripts/test_client.py` is gone:

```bash
ls scripts/test_client.py 2>&1
```
Expected: "No such file or directory".

- [ ] Run existing mock tests to confirm no regressions:

```bash
python -m pytest tests/test_protocol.py tests/test_datetime_patch.py tests/test_client_exceptions.py tests/test_auth_limiter.py -v -q
```
Expected: all pass.
